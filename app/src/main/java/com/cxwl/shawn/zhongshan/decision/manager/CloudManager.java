package com.cxwl.shawn.zhongshan.decision.manager;
...
public class CloudManager {
	...
	public void loadImagesAsyn(List<RadarDto> radars, CloudListener listener) {
		if (mLoadThread != null) {
			mLoadThread.cancel();
			mLoadThread = null;
		}
	...
	private class LoadThread extends Thread {
		...
		private LoadThread(List<RadarDto> radars, CloudListener listener) {
			this.radars = radars;
		...
		private String decodeFromUrl(String url, int index){
		    try {
				...
				try {
					File file = new File(getDir() + "/cloud"+index+".png");
		...
		private synchronized void finished(String path, List<RadarDto> radars) {
			...
			if (listener != null) {
				...
				if (count <= 0) {
					listener.onResult(radars == null ? CloudListener.RESULT_FAILED : CloudListener.RESULT_SUCCESSED, radars);
				}
			}
		}
